...
class TestUserViewSet(TestCase):
    ...
    def test_get_list(self):
        ...
        view = UserMainModelView.as_view({'get': 'list'})
    ...
    def test_create_user_factory(self):
        ...
        view = UserMainModelView.as_view({'post': 'create'})
        admin = UserModelSet.objects.create_superuser('admin','admin@admin.ru','1')
        force_authenticate(request,admin)
        response = view(request)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_get_detail(self):
        user = UserModelSet.objects.create(username='test', email=self.email_test())
        client = APIClient()
        response = client.get(f'/api/user/{user.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_create_user_client(self):
        UserModelSet.objects.create_superuser('admin', 'admin@admin.ru', '1')
        client = APIClient()
